<?php get_header(); ?>
<section class="under-hero-section"></section>
  <section id="top" class="hero-section">
    <div class="top-texts">
      <h1>一関シュトーレン</h1>
      <p>Ichinoseki stollen</p>
    </div>
    
    <div class="top-catch-message">
      <div class="ja-context">クリスマスのお祝いに<br>極上のシュトーレンを<br></div>
      <div class="en-context">Enjoy the finest stollen<br> for your Christmas celebration<br></div>
    </div>
    <div class="scroll-button">
      <div class="scroll">SCROLL</div>
      <div class="icon-embed-xxsmall w-embed"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="iconify iconify--ic" width="100%" height="100%" preserveaspectratio="xMidYMid meet" viewbox="0 0 24 24">
          <path fill="currentColor" d="M11.67 3.87L9.9 2.1L0 12l9.9 9.9l1.77-1.77L3.54 12z"></path>
        </svg></div>
    </div>
  </section>
  <section class="second-section">
    <div class="sec-section-inner-message">一つ一つの食材を<br>農家さんにお願いしてご用意していただきました。</div>
  </section>
  <div class="side-menu"><img src="<?php bloginfo('template_url');?>/images/logo-white.png" loading="lazy" sizes="(max-width: 767px) 100vw, 15vw" class="menu-logo">
    <ul role="list" class="list w-list-unstyled">
      <li id="nav" class="menu-item link-anime">
        <a href="#top" class="top-link link-anime">TOP</a>
      </li>
      <li id="nav" class="menu-item">
        <a href="#stollen" class="content-link">一関シュトーレン</a>
      </li>
      <li id="nav" class="menu-item">
        <a href="#shop" class="shop-link">ご予約はこちら</a>
      </li>
      <li id="nav" class="menu-item">
        <a href="https://forms.gle/RoFkBgsDaqZmZVPs5" class="contact-link">問い合わせ</a>
      </li>
      <li id="nav" class="menu-item">
        <!--20231026変更-->
        <a href="https://www.instagram.com/kaziya1699/" class="kaziya-link header"> Café &amp; Guesthouse Kaziya</a>
      </li>

      <!--20231026追加-->
      <li id="nav" class="menu-item">
        <a href="https://www.instagram.com/cafelounge_vin_gogh/" class="kaziya-link header"> Cafe Lounge vin gogh</a>
      </li>

    </ul>
    <div class="navbar-logo-center">
      <div data-animation="default" data-collapse="medium" data-duration="400" data-easing="ease" data-easing2="ease" role="banner" class="navbar-logo-center-container shadow-three w-nav">
        <div class="container">
          <div class="navbar-wrapper-three">
            <nav role="navigation" class="nav-menu-wrapper-three w-nav-menu">
              <div class="nav-menu-three">
                <ul role="list" class="nav-menu-block w-list-unstyled">
                  <li class="list-item">
                    <a href="#top" class="nav-link-2 w-nav-link">TOP</a>
                  </li>
                  <li>
                    <a href="#stollen" class="nav-link-2">一関シュトーレン</a>
                  </li>
                  <li>
                    <a href="#shop" class="nav-link-2">ご予約はこちら</a>
                  </li>
                  <li>
                    <a href="https://forms.gle/RoFkBgsDaqZmZVPs5" class="nav-link-2">問い合わせ</a>
                  </li>
                  <li>
                    <!--20231026変更-->
                    <a href="https://www.instagram.com/kaziya1699/" class="nav-link-2"> Café &amp; Guesthouse Kaziya</a>
                  </li>
                  <li>
                    <!--20231026追加-->
                    <a href="https://www.instagram.com/cafelounge_vin_gogh/" class="nav-link-2"> Cafe Lounge vin gogh</a>
                  </li>
                </ul>
              </div>
            </nav>
            <div class="menu-button w-nav-button">
              <div class="w-icon-nav-menu"></div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <section id="stollen" class="third-section">
    <div class="second-inner-block">
      <div class="thir-section-img-below first-below">
        <!-- remove image above 11/7 -->
        <!-- <div class="thir-section-img-above first-picture"></div> -->
      </div>
    </div>
    <div class="second-inner-block">
      <!-- テキスト変更　11/6 -->
      <div class="text-block-3"><span class="first-typography">小麦粉は岩手県一関市大東町産の<br>完全無農薬ゆきちから。</span><br><span class="second-typography">フルーツは干し柿、林檎、無花果、栗などの<br>和のフルーツ。<br>お砂糖は東北の食材と相性のいい甜菜糖。</span></div>
    </div>
  </section>
  <section class="fourth-section">
    <div class="second-inner-block">
      <!--20231026変更-->
      <div class="thir-typography"><span class="text-span-below">ワンサイズ（250g)のみのご用意となります。<br>お歳暮や、手土産にもおすすめです。<br>賞味期限は翌年1月31日となります。</span></div>
    </div>
    <div class="second-inner-block">
      <div class="thir-section-img-below">
        <!-- remove image above 11/7 -->
        <!-- <div class="thir-section-img-above second-pictures"></div> -->
      </div>
    </div>
  </section>
  <section id="shop" class="fifth-section">
    <div class="price-box">
      <!--20231026変更-->
      <div class="merchandise">一関シュトーレン</div>
      <div class="price">￥ <span class="price-core">3,300</span>(税込)</div>
      <div class="text-block-7">
        <a href="https://forms.gle/cZSjDPii3kHaQnA37" class="inner-shop-link">ご予約はこちら</a>
      </div>
    </div>
    <div class="img-back-section">
      <!--20231026変更-->
      <div class="img-back-inner-context">一口たべると一関の食材の持つ<br>
                                          優しい風味が広がります。<br>
                                          時の経過とともに熟成される風味を<br>
                                          お楽しみください。</div>
    </div>
  </section>
  <section class="sixth-section">
    <p class="section-title">シュトーレンとは</p>
    <p class="first-state">
    　「シュトーレン」とはドイツの伝統的なお菓子です。 日持ちするうえ、日に日に変化していく味わいを楽しめます。 
    </p>
    <div class = "stollen-image">
    </div>
    <p class="second-state">
      　ドイツではクリスマスの4週間の間に、シュトーレンを薄くスライスして少しずつ食べる習慣があります。 シュトーレンをスライスするときは、端から食べていくのではなくまず真ん中から半分に切り、その日に食べる分だけ中央部分から切り分けます。
    </p>
    <p class="third-state">
      　残ったシュトーレンは、切り口同士を合わせ、空気に触れないようにラップなどで包んで保存します。
    </p>
    <p class="fourth-state">
      　コーヒーや紅茶と相性がよいのはもちろん、赤ワインなどのお酒ともよく合います。
    </p>
  </section>
  <section class="seventh-section">
    <p class="title">商品受け取り方法</p>
    <div class="shop-information">
      <div class="information-title">
        <div class="title-logo"></div>
        <p class="text">店舗受取り</p>
      </div>
      <p>　以下の二店舗にて商品を受け取ることが出来ます。</p>
      <div class="shops">
        <div class="kaziya-shop">
          <div class="shop-logo logos"></div>
          <p class="access">アクセス　　　：〒029-0302<br>
                            　　　　　　　　岩手県一関市 東山町長坂町43</p>
          <p class="tel">   電話番号　　　：0191-34-5621</p>
          <p class="time">  受取り可能時間：平日・祝日（土日を除く）　15:00 ~ 18:00<br>
                            　　　　　　　　土・日　　11:00 ~ 18:00
          </p>
          <div class="googlemap"><a href="https://maps.app.goo.gl/dg612YFypKyoXmwAA">OPEN IN GOOGLE MAP<div class="googlemap-logo"></div></a></div>
        </div>
        <div class="gogh-shop">
          <div class="shop-logo logos"></div>
          <p class="access">アクセス　　　：〒029-3205<br>
                            　　　　　　　　岩手県一関市花泉町涌津亥年前16</p>
          <p class="tel">   電話番号　　　：070-8468-7657</p>
          <p class="time">  ※17時以降をご希望の場合は，お電話ください。
          </p>
          <div class="googlemap"><a href="https://maps.app.goo.gl/ugykmxcGzvZrp1f78">OPEN IN GOOGLE MAP<div class="googlemap-logo"></div></a></div>
        </div>
      </div>
      
    </div>
    <div class="delivery-information">
      <div class="information-title">
        <div class="title-logo"></div>
        <p class="text">配送（佐川急便）</p>
      </div>
      <p class="postage">　送料は一律700円（税込み）となります。</p>
    </div>
  </section>
  <?php get_footer(); ?>